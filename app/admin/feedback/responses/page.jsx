 ...
 * 
 * Displays a paginated table of feedback responses with search, delete and view capabilities.
...
const ResponsesPage = () => {
  ...
  usePageTitle({
    title: [
      ROUTES.ADMIN_FEEDBACK_RESPONSES.name,
      ...
      <ADMIN_COMPONENTS.DataTable.Table
        ...
        renderRowActions={({ row }) => (
          <ADMIN_COMPONENTS.DataTable.RowActions.Main
            actions={(() => {
              const { DeleteAction, ViewAction } = ADMIN_COMPONENTS.DataTable.RowActions;
              return [
                DeleteAction({ onClick: () => handleDelete(row.original._id) }),
                ViewAction({ href: `${ROUTES.ADMIN_FEEDBACK_RESPONSES.path}/${row.original._id}?view=true` })
              ];
            })()}
          />
        )}
...
ResponsesPage.getLayout = (page) => <AdminLayout>{page}</AdminLayout>;
